...
def handle_get_likes(
    ...
    with postgres_conn.cursor() as postgres:
        try:
            liked = check_if_liked(video_id, user['user_id'], postgres)
            likes_count = get_likes_count(video_id, postgres)
        except Exception as error:
